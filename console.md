---
description: >-
  This documentation provides information about the front-end console of the
...
Thinger.io is a cloud platform, so it is mainly based on a backend program that performs all device administration tasks and manage devices data traffic. All these features can be easily configured using the "Web console", a frontend created to be used with any multimedia device through a web browser. This frontend also offers a customizable dashboards system to provide data representation and graphical analytics or even creating control dashboards using action widgets.

The next image shows an overview of how does the workspace looks like just after login, displaying the `Statistics` section, with some basic information about the account status, like number of devices, endpoints, data buckets, endpoints, and so on. There is also a map that shows the approximate location of each connected devices according their IP address or GPS location if provided. Finally, there are some statistics about your devices consumption in terms of sent or received information.
